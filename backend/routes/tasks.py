...
async def create_tasks(task: create_tasks, db:Session=Depends(get_db)):
...
async def update_tasks(task_id: int, updated_task: create_tasks, db: Session = Depends(get_db)):
